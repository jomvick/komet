...
//!
//! ```sh
//! KOMET_EDGE_WS=ws://127.0.0.1:27640 cargo test -p komet-sync --test registry_edge -- --ignored
//! ```
...
use komet_doc::RegistryDoc;
use komet_proto::{Chat, Session, SessionStatus};
use komet_sync::RegistryClient;
...
fn edge_url(org: &str, user: &str) -> String {
    let base = std::env::var("KOMET_EDGE_WS")
        .expect("set KOMET_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:27640");
...
#[tokio::test]
#[ignore = "requires a live edge: set KOMET_EDGE_WS (e.g. ws://127.0.0.1:27640)"]
