//! Workspace doc schema over `loro` — the per-org entity index that replaces komet's
...
//! sets from any device — matching komet's Mutate surface. Presence rides the room's
//! `EphemeralStore` under keys `presence/{deviceId}` (an online timestamp), replacing
//! komet's 15s heartbeat writes so liveness never grows the oplog.
//!
//! Timestamps are stored as epoch millis (the session-doc convention) and surface as
//! `chrono::DateTime<Utc>` through the `komet_proto` entity types.

use chrono::{DateTime, Utc};
use loro::{ExportMode, LoroDoc, LoroMap, LoroValue, ToJson};
use serde::{Deserialize, Serialize};

use komet_proto::{Chat, ChatConfig, Device, Session, SessionStatus, Space};
...
impl WorkspaceDoc {
    ...
    /// latest run and the cwd it was created under (komet stored the same pair
...
mod tests {
    use super::*;
    use komet_proto::{HarnessId, SandboxLevel};
