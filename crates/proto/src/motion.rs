//! Loader motion — the pure math behind komet's loading indicators.
//!
//! These are the curves and constants the gpui viewport animates with
//! (`komet-ui/src/motion.rs`, `komet-ui/src/loaders.rs`), lifted here so any
...
/// Komet loader pulse period.
pub const KOMET_PULSE_MS: u64 = 2_400;
/// Gradient matrix spinner wave period.
pub const GRADIENT_SPIN_MS: u64 = 750;

/// Cells in the komet wave loader.
pub const KOMET_CELLS: usize = 5;
/// Side length of the gradient spinner matrix.
pub const MATRIX_SIDE: usize = 3;

/// Komet loader cells rest at this opacity between pulses.
...
/// Per-row tints of the gradient matrix spinner — komet's "sunrise" gradient
...
/// Komet loader cell opacity for a phase: 0.08 → 1 → 0.08.
pub fn pulse_opacity(phase: f32) -> f32 {
    PULSE_MIN_OPACITY + (1.0 - PULSE_MIN_OPACITY) * pulse_wave(phase)
}

/// Komet loader cell scale for a phase: 0.9 → 1 → 0.9.
...
/// ported from komet's `gradient-spin-pulse` keyframes: full at the cycle
...
/// The komet mark's pixels — `[x, y]` of each 100×100 cell on the 820×940
/// canvas (komet's `logo.tsx` CELLS), shared by the static mark and the
/// animated loader.
#[rustfmt::skip]
...
/// Per-cell stagger along the komet's flight axis. The stagger *adds* phase
