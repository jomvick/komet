...
//!
//! In komet these were synced Postgres rows; in komet they live in the per-org
...
pub struct Device {
    ...
    /// First registration time (komet devices.created_at — the Devices page
...
pub struct Chat {
    ...
    /// continuity across engine restarts (komet's `chats.harness_session_id`).
...
pub struct Worktree {
    ...
    pub path: String,
    pub branch: String,
    /// Generated worktree folder name (`komet/<name>` is its branch).
    #[serde(default)]
    pub name: String,
        ...
        for (state, encoded_state) in [
            ...
            let status = CheckoutChangeRequestStatus {
                ...
                change_request: Some(ChangeRequestSummary {
                    ...
                    url: "https://github.com/acme/komet/pull/90".into(),
                    state,
                    base_ref: "main".into(),
