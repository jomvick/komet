...
impl AuthConfig {
    pub fn new(edge_url: impl Into<String>, data_dir: impl Into<PathBuf>) -> Self {
        Self {
            ...
            sync_token: std::env::var("KOMET_SYNC_TOKEN")
                .ok()
...
impl Auth {
    pub fn new(config: AuthConfig) -> Self {
        let token = config.sync_token.clone().or_else(|| {
            std::env::var("KOMET_SYNC_TOKEN")
                .ok()
                .filter(|s| !s.trim().is_empty())
        });
        let user_id = if config.dev_user_id.trim().is_empty() {
            "local".to_string()
        } else {
    ...
    pub async fn start_sign_in(&self) -> Result<String, crate::EngineError> {
        Ok(String::new())
    }
    pub fn start_headless_sign_in(&self) -> String {
        String::new()
    }
    pub async fn complete_sign_in(&self, _pasted: &str) -> Result<(), crate::EngineError> {
        Ok(())
    }
    pub fn sign_out(&self) {}
    pub async fn list_orgs(&self) -> Result<Vec<OrgMembership>, crate::EngineError> {
        Ok(vec![])
    }
    pub async fn create_org(&self, _name: &str) -> Result<(), crate::EngineError> {
        Ok(())
    }
    pub async fn select_org(&self, _organization_id: &str) -> Result<(), crate::EngineError> {
        Ok(())
    }
}
