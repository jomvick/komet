...
//! komet's `terminals.ts` over `portable-pty`).
//!
...
//!   owner, so the per-user owner re-checks from komet's Router land with real
...
use komet_doc::TERMINAL_OUTPUT_BATCH_MS;
