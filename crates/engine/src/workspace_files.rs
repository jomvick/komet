...
fn is_internal_temp_wire_path(path: &str) -> bool {
    path.rsplit('/')
        .next()
        .is_some_and(|name| name.starts_with(".komet-save-") && name.ends_with(".tmp"))
}
...
fn write_file_blocking(
    ...
    let temp_path = parent.join(format!(".komet-save-{}.tmp", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    fn watch_normalizes_renames_deduplicates_and_filters_git() {
        ...
        let events = vec![
            ...
            Ok(
                notify::Event::new(EventKind::Modify(ModifyKind::Name(RenameMode::Both)))
                    .add_path(root.join(".komet-save-dead.tmp"))
