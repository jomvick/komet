...
//! (`KOMET_SELF_TURN_QUIESCE_MS`); prompt/steer turns keep the normal one.
//!
...
fn init_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe {
            std::env::set_var("KOMET_TURN_QUIESCE_MS", QUIESCE_MS.to_string());
            std::env::set_var("KOMET_SELF_TURN_QUIESCE_MS", SELF_QUIESCE_MS.to_string());
        }
    });
}
