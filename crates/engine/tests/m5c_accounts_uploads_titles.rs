...
use komet_proto::{AgentAccountsSnapshot, AgentEvent, DoneStatus, HarnessId, SandboxLevel};
...
async fn uploads_chunk_commit_readback_and_jail() {
    ...
    // Non-image extensions are refused even inside the jail (komet parity).
...
async fn titling_e2e_names_chat_and_renames_worktree_branch() {
    ...
    let request = komet_proto::RunRequest {
    ...
    assert_eq!(chat.branch.as_deref(), Some("komet/fix-login-flow"));
...
async fn rename_worktree_branch_guards_and_collisions() {
    ...
    let unchanged = repos
        .rename_worktree_branch(wt_path, "komet/not-this-one", "Some Title")
        .await
        .expect("guarded");
    ...
    let renamed = repos
        ...
        .await
        .expect("renamed");
    assert_eq!(renamed, "komet/add-dark-mode");

    // Already renamed → the guard (branch no longer komet/<folder>) makes any
    // further title rename a no-op.
    let again = repos
        .rename_worktree_branch(wt_path, "komet/add-dark-mode", "Different Title")
        .await
        .expect("second rename");
    assert_eq!(again, "komet/add-dark-mode");
    ...
    assert!(
        renamed2.starts_with("komet/add-dark-mode-")
            && renamed2.len() == "komet/add-dark-mode-".len() + 6,
        "suffixed: {renamed2}"
    );
    ...
    assert_eq!(worktree_branch_from_title("***"), "komet/update");
    assert_eq!(
        worktree_branch_from_title("Cafe's Dark Mode"),
        "komet/cafes-dark-mode"
    );
}
