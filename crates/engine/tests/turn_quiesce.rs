...
//!    existed in the agent's session data, was absent from komet's doc).
...
use komet_engine::{EngineCore, HarnessRegistry, SteerOutcome};
...
fn init_quiesce_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe { std::env::set_var("KOMET_TURN_QUIESCE_MS", QUIESCE_MS.to_string()) };
    });
}
