...
//! variant against a real edge runs behind `#[ignore]` (KOMET_EDGE_WS, like
//! komet-sync's edge_convergence test).
...
use komet_doc::{CommandBasedOn, SessionCommandEntry, SessionCommandPayload, SessionCommandStatus};
...
async fn bridge(
    a: &EngineCore,
    b: &EngineCore,
) -> komet_sync::registry::mock_server::MockRegistryServer {
    let server = komet_sync::registry::mock_server::MockRegistryServer::start().await;
...
async fn two_engines_share_a_workspace() {
    ...
    let client_a = komet_rpc::memory_client(a.rpc_service());
    let client_b = komet_rpc::memory_client(b.rpc_service());
    client_a
        .call(
            methods::MUTATE,
            serde_json::json!({
    ...
    wait_for(
        || {
            handle.doc().read_entries().unwrap_or_default().iter().any(|e| {
                e.parts.iter().any(
                    |p| matches!(p, komet_doc::MessagePart::Text { text, .. } if text == "From cursor"),
                )
            })
        },
...
///
/// ```sh
/// KOMET_EDGE_WS=ws://127.0.0.1:8787 cargo test -p komet-engine -- --ignored
/// ```
#[tokio::test]
#[ignore = "requires a live edge: set KOMET_EDGE_WS (e.g. ws://127.0.0.1:8787)"]
async fn two_engines_converge_through_a_real_workspace_room() {
    use komet_engine::doc_host::EdgeConfig;

    let base = std::env::var("KOMET_EDGE_WS")
        .expect("set KOMET_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:8787");
...
async fn legacy_workspace_doc_migrates_instantly_on_first_boot() {
    use komet_proto::{Chat, Device, Session, Space};
    ...
    {
        let store = komet_sync::DocsStore::open(&org_dir).expect("open store");
        let legacy = komet_doc::WorkspaceDoc::new();
    ...
    let store = komet_sync::DocsStore::open(&org_dir).expect("reopen store");
    assert!(
        store
            .load_snapshot(komet_doc::REGISTRY_DOC_ID)
