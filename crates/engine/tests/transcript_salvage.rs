...
use komet_doc::{MessagePart, MessageRole, MessageStatus, SessionDoc, SessionMessageEntry};
...
async fn blank_journaled_chat_recovers_entries_from_fat_rollback() {
    ...
    {
        let store = komet_sync::DocsStore::open(&org_dir).unwrap();
        store
