...
use komet_proto::{GitHistoryRefKind, TerminalEvent};
...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: komet/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("komet/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's komet/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, komet branch removed, refs pruned.
    repos
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = komet_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Komet diff truncated"));
}
