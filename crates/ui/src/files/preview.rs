...
struct HighlightedFile {
    content_hash: String,
    document: Arc<komet_syntax::HighlightedDocument>,
}
    ...
    std::mem::size_of::<HighlightedFile>()
        ...
        .saturating_add(
            document
                .lines
                .capacity()
                .saturating_mul(std::mem::size_of::<Vec<komet_syntax::HighlightSpan>>()),
        )
        .saturating_add(document.lines.iter().fold(0usize, |total, line| {
            total.saturating_add(
                line.capacity()
                    .saturating_mul(std::mem::size_of::<komet_syntax::HighlightSpan>()),
            )
        }))
}
...
impl FilesSurface {
    ...
    fn request_file_highlight(
        ...
        let Some(language) = komet_syntax::language_for_path(&path) else {
            return;
        };
                    ...
                    .on_click(cx.listener(move |this, _, _, cx| {
                        ...
                        this.reveal_search_result(
                            WorkspaceFileSearchMatch {
                                ...
                                name,
                                kind: komet_proto::WorkspaceEntryKind::File,
                                score: 0,
                            },
                            cx,
                        );
