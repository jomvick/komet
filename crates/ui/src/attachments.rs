...
//!
//! Ports of komet's `composer/use-attachments.ts` (staging/upload),
...
/// Base64 chars per `UploadChunk` (komet state.ts `UPLOAD_CHUNK` — sized for
...
/// `(body_end, refs_start)` byte offsets — the tolerant equivalent of komet's
/// `ATTACHED_IMAGES_RE`.
...
/// the same stuck-offset guard as komet's `readAttachmentImage`).
