...
//! `remend`, ported to komet's incremental parser).
//!
...
//! handlers don't apply here — komet renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "komet:pending-link";
...
pub fn close_hanging(text: &str) -> Option<String> {
    ...
    pending.sort_by_key(|&(pos, _)| std::cmp::Reverse(pos));
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](komet:pending-link)");
        mends("[docs](", "[docs](komet:pending-link)");
        mends("see [do", "see [do](komet:pending-link)");
        mends("![alt](https://x/i.p", "![alt](komet:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](komet:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](komet:pending-link)");
        mends("**a [b", "**a [b](komet:pending-link)**");
    }
