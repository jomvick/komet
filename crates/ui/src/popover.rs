...
use crate::motion::{self, KOMET_PULSE};
...
/// The floating-menu surface (komet `.glass-surface` + `menuSurface`):
...
/// One menu row (komet `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (komet's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (komet
...
/// Hairline divider between menu sections (komet `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (komet
...
/// A bordered trailing menu section (komet picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (komet dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (komet dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (komet:
