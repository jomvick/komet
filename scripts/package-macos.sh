...
#   target/package/komet-<version>-macos-<arch>.dmg          (user download)
#   target/package/komet-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Komet.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Komet.app"
DMG="$OUT_DIR/komet-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/komet-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/komet" "$APP/Contents/MacOS/komet"
...
# applied ahead of time; dist/komet.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/komet.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/komet.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/komet-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Komet.app
echo "packaged: $APP_TARBALL"

# The dmg presents the classic drag-into-Applications layout over the
# backdrop artwork (committed renders from scripts/dmg-background.py).
...
dmgbuild.build_dmg(
    ...
    settings={
        ...
        "icon": os.path.join(app, "Contents/Resources/komet.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Komet.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
