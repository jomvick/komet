...
- EphemeralStore (loro::awareness::EphemeralStore): timestamp-LWW presence store, per-key timeout
  ...
  Rust and JS. (Komet's %EPH presence maps to this.)
...
- Plan: build a small `komet-mirror` crate: typed schema structs + incremental diff application over
...
- Sync caveat: peers can only sync if their version is after the shallow start — the session DO is
  the natural sequencer for compaction (komet already does daily frontier checkpoints >= RETAIN_DAYS).
...
- Watch: #940 (storage-backed LoroDoc, open), #1040 (shallow-snapshot import w/ concurrent ops can
  stall pending — compact at quiesced points; komet's daily-checkpoint scheme should respect this).
