...
- Add an **ACP harness** (`crates/harness/src/acp/`) speaking Agent Client Protocol
  ...
  NOT the official SDK crates: komet keeps its own child-lifecycle hardening
...
- First registered agent: **Grok Build** (`grok agent stdio`), xAI's native ACP
  ...
  OAuth or `XAI_API_KEY`; komet passes env through. `GROK_EXECUTABLE` overrides
...
- **claude/codex converted to ACP too** (2026-08-08, wing's call: "keep things
  ...
  adapter-owned; komet-specific settings ride config options where advertised
...
- **Hermes + Pi registered** (2026-08-08): `AcpHarness::hermes()` runs Nous
  ...
  entry); thinking ladder minimal→max maps onto komet's levels via the
  generic `thought_level` preference ladder ("off" has no komet tier).
...
- **Session config options**: ACP has no per-prompt model field; the run's
  ...
  `low`/`medium`/`high`; other komet levels degrade down a preference ladder
  (`config_option_sets`).
...
- Replaced by `adapter_install`: pinned packages install ONCE into
  `~/.komet/adapters/<pkg>/<version>` (`$KOMET_ADAPTERS_DIR` overrides) with
  a komet-owned npm cache beside them, atomically (tmp dir + bin-entry
  verification + `.komet-install-ok` marker + rename), then every launch is
